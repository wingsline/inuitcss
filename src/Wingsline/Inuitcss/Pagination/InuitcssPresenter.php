...
namespace Wingsline\Inuitcss\Pagination;

use Illuminate\Pagination\Paginator;

class InuitcssPresenter
{
    ...
    /**
     * Create a new Inuitcss presenter instance.
     *
    ...
    /**
     * Render the Inuitcss pagination contents.
     *
     * @return string
     */
    ...
    /**
     * Render the Inuitcss pagination contents without prev or next
     *
     * @return string
     */
    ...
    public function getPageRange($start, $end)
    {
        ...
        for ($page = $start; $page <= $end; $page++) {
            ...
            // for the link. These views use the "Inuitcss" styles by default.
     ...
     * @return string
     */
    public function getPrevious($text = '&lsaquo;')
    {
        // If the current page is less than or equal to one, it means we can't go any
        // further back in the pages, so we will render a disabled previous button
        // when that is the case. Otherwise, we will give it an active "status".
        if ($this->currentPage <= 1) {
            return '<li class="pagination__prev">'.$text.'</li>';
        } else {
            $url = $this->paginator->getUrl($this->currentPage - 1);

            return '<li class="pagination__prev"><a href="'.$url.'">'.$text.'</a></li>';
        }
    }
    ...
    public function getFirst($text = '')
    {
        ...
        // when that is the case. Otherwise, we will give it an active "status".
        if ($this->currentPage <= 1) {
            return '<li class="pagination__first">&laquo; '.$text.'</li>';
        } else {
            $url = $this->paginator->getUrl(1);

            return '<li class="pagination__first"><a href="'.$url.'">'.$text.'</a></li>';
        }
    }
     ...
     * @return string
     */
    public function getNext($text = '&rsaquo;')
    {
        // If the current page is greater than or equal to the last page, it means we
        // can't go any further into the pages, as we're already on this last page
        // that is available, so we will make it the "next" link style disabled.
        if ($this->currentPage >= $this->lastPage) {
            return '<li class="pagination__next">'.$text.'</li>';
        } else {
            $url = $this->paginator->getUrl($this->currentPage + 1);

            return '<li class="pagination__next"><a href="'.$url.'">'.$text.'</a></li>';
        }
    }
    ...
    {
        // If the current page is less than or equal to one, it means we can't go any
        // further back in the pages, so w$this->lastPage$this->lastPagee will render a disabled previous button
        // when that is the case. Otherwise, we will give it an active "status".
        if ($this->currentPage >= $this->lastPage) {
            return '<li class="pagination__last">'.$text.' &raquo;</li>';
        } else {
            $url = $this->paginator->getUrl($this->lastPage);

            return '<li class="pagination__last"><a href="'.$url.'">'.$text.'</a></li>';
        }
    }
    ...
    {
        return '<li class="disabled">&hellip;</li>';
    }
